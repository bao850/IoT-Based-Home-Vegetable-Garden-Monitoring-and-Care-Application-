...
// ================= FIREBASE PATH =================
const CTRL   = "smart_garden/control/vuon2";
const CTRL_SHARED = "smart_garden/control/vuon1";   // đèn + phun chung
...
// ================= IMAGE PATH =================
const RAW  = "/smart_garden/image/vuon2";
const CAPTURE_REQ = "/smart_garden/camera/capture2";
const CROP = "/smart_garden/image_crop/vuon2";
const CLOUD_NAME = "djqp5gxru";
const UPLOAD_PRESET = "garden1";                 // preset của bạn
const CLOUD_FOLDER = "garden2_crop";             // ✅ folder lưu ảnh crop
...
const SENSOR = "smart_garden/sensor/vuon2";
const MODE = "smart_garden/mode/vuon2";
...
// ===== CLICK ICON → ĐẢO TRẠNG THÁI =====
bulb.onclick = () => {
  if (window.autoMode === 1) return;
  database.ref(`${CTRL_SHARED}/bulb`).set(bulbState ? 0 : 1);
};
...
phun.onclick = () => {
  if (window.autoMode === 1) return;
  database.ref(`${CTRL_SHARED}/spray`).set(sprayState ? 0 : 1);
};

// ===== FIREBASE → CẬP NHẬT ICON =====
database.ref(`${CTRL_SHARED}/bulb`).on("value", s => {
  bulbState = Number(s.val() ?? 0);
  bulb.src = bulbState ? "./img/ledon.png" : "./img/ledoff.png";
});
...
database.ref(`${CTRL}/fan`).on("value", s => {
  ...
  // ❌ KHÔNG ĐỔI SRC Ở ĐÂY NỮA
  fanImg.src = "./img/cq.png";
...
database.ref(`${CTRL}/pump`).on("value", s => {
  ...
  pump.src = pumpState ? "./img/pumpon.png" : "./img/pumpoff.png";
});

database.ref(`${CTRL_SHARED}/spray`).on("value", s => {
  sprayState = Number(s.val() ?? 0);
  phun.src = sprayState ? "./img/phunon.png" : "./img/phunoff.png";
});
...
function updateLockGarden2() {
  const anyAuto = (mode1 === 1 || mode2 === 1 || mode3 === 1);

  // 🔒 ĐÈN + PHUN (dùng chung)
  [bulb, phun].forEach(icon => {
    icon.style.pointerEvents = anyAuto ? "none" : "auto";
    icon.style.opacity       = anyAuto ? "0.5"  : "1";
  });

  // 🔒 QUẠT + BƠM VƯỜN 2 (riêng)
  const lockGarden2 = (mode2 === 1);
  [fan, pump].forEach(icon => {
    icon.style.pointerEvents = lockGarden2 ? "none" : "auto";
    icon.style.opacity       = lockGarden2 ? "0.5"  : "1";
  });
}
...
database.ref(RAW).on("value", async (snap) => {
  ...
  const cropUrl = rawUrl.replace(
    "/upload/",
    "/upload/c_crop,fl_relative,w_0.3,h_2,x_0,y_0.6/"
  );
...
// ============== CẬP NHẬT SENSOR ==================
// DÙNG CHUNG TỪ VƯỜN 1
database.ref("smart_garden/sensor/vuon1/temp").on("value", s=>{
  const data = s.val() || {};
  document.getElementById("tempt").innerText = fmtTemp(data.live) + "°C";

  temptChart = mapChart(data);
...
database.ref("smart_garden/sensor/vuon1/humi").on("value", s=>{
  const data = s.val() || {};
  document.getElementById("humi").innerText = fmtInt(data.live) + "%";

  humidChart = mapChart(data);
...
database.ref("smart_garden/sensor/vuon1/light").on("value", s=>{
  const data = s.val() || {};
  document.getElementById("light").innerText = fmtInt(data.live) + " Lux";

  lightChartData = mapChart(data);
...
// RIÊNG ĐỘ ẨM ĐẤT VƯỜN 2
database.ref("smart_garden/sensor/vuon2/soil").on("value", s=>{
  const data = s.val() || {};
  document.getElementById("soil").innerText = fmtInt(data.live) + "%";

  soilChartData = mapChart(data);
...
function showChart(evt, name) {
  ...
  if (name === "light") {
    mainChart.updateSeries([{ name: "Light", data: lightChartData }]);
    mainChart.updateOptions({
  ...
  if (name === "soil") {
    mainChart.updateSeries([{ name: "Soil Moisture", data: soilChartData }]);
    mainChart.updateOptions({
...
database.ref(MODE).on("value", s => {
  const mode = Number(s.val() ?? 0);
  window.autoMode = mode;

  autoBtn.classList.toggle("active",   mode === 1);
  manualBtn.classList.toggle("active", mode === 0);
