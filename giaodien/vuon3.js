...
// ================= FIREBASE PATH =================
const CTRL   = "smart_garden/control/vuon3";
const CTRL_SHARED = "smart_garden/control/vuon1";   // đèn + phun chung
const SENSOR = "smart_garden/sensor/vuon3";
const MODE = "smart_garden/mode/vuon3";

// ================= IMAGE PATH =================
const RAW  = "/smart_garden/image/vuon3";
const CAPTURE_REQ = "/smart_garden/camera/capture3";
const CROP = "/smart_garden/image_crop/vuon3";
const CLOUD_NAME = "djqp5gxru";
const UPLOAD_PRESET = "garden1";                 // preset của bạn
const CLOUD_FOLDER = "garden3_crop";             // ✅ folder lưu ảnh crop
...
database.ref(MODE3).on("value", s => {
  ...
  window.autoMode = mode3;

  autoBtn.classList.toggle("active",   mode3 === 1);
  manualBtn.classList.toggle("active", mode3 === 0);
...
database.ref(RAW).on("value", async (snap) => {
  ...
  const cropUrl = rawUrl.replace(
    "/upload/",
    "/upload/c_crop,fl_relative,w_1,h_2,x_0.7,y_0.6/"
  );
...
// RIÊNG ĐỘ ẨM ĐẤT VƯỜN 3
database.ref("smart_garden/sensor/vuon3/soil").on("value", s=>{
  const data = s.val() || {};
...
function updateLockGarden3() {
  ...
  // 🔒 QUẠT + BƠM VƯỜN 3 (riêng)
  const lockGarden3 = (mode3 === 1);
  [btnFan, btnPump].forEach(btn => {
    btn.style.pointerEvents = lockGarden3 ? "none" : "auto";
    btn.style.opacity       = lockGarden3 ? "0.6"  : "1";
  });
}
updateLockGarden3();
